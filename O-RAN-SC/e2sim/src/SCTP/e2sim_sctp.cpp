...
int sctp_send_data(int &socket_fd, sctp_buffer_t &data)
{
  ...
  // pid_t pid = getpid(); // Using process ID to add another layer of uniqueness
  // snprintf(filename, sizeof(filename), "sctp_data_%ld_%d.bin", now, pid);

  // // Save the buffer to a binary file
  // FILE *file = fopen(filename, "wb");
  // if (!file) {
  //   perror("[SCTP] Error opening file");
  //   exit(1);
  // }
  // fwrite(data.buffer, 1, data.len, file);
  // fclose(file);
