...
int main(int argc, char *argv[])
{
	...
	e2sim.register_subscription_callback(2, &callback_kpm_subscription_request);
...
// {
// 	LOG_I("Starting report loop");
// 	long seqNum = 1; // Starting sequence number
// 	srand((unsigned int)time(NULL));
// 	while (1)
// 	{
// 		LOG_I("Sending measurement report for sequence %d", seqNum);

// 		int server_fd = socket(AF_INET, SOCK_STREAM, 0);
// 		if (server_fd == 0)
// 		{
// 			LOG_E("kpm: Socket failed\n");
// 			exit(EXIT_FAILURE);
// 		}
...
// 		address.sin_port = htons(19960);

// 		// Bind socket to port
// 		if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) < 0)
// 		{
// 			LOG_E("kpm: Bind failed\n");
// 			continue;
// 		}
...
// 		{
// 			LOG_E("kpm: Listen failed\n");
// 			exit(EXIT_FAILURE);
// 		}

// 		int addrlen = sizeof(address);
// 		LOG_I("kpm: Waiting for connection\n");

// 		// Accept a client connection
// 		int new_socket = accept(server_fd, (struct sockaddr *)&address, (socklen_t *)&addrlen);
// 		if (new_socket < 0)
// 		{
// 			LOG_E("kpm: Accept failed\n");
// 			exit(EXIT_FAILURE);
// 		}

// 		LOG_I("kpm: Connection accepted\n");

// 		char buffer[2 << 20] = {0};
// 		int valread;
// 		std::string receivedData;

// 		// Continuously read from the socket until the client closes the connection or the size limit is reached
// 		while ((valread = recv(new_socket, buffer, sizeof(buffer), 0)) > 0)
// 		{
// 			receivedData.append(buffer, valread);
// 			if (receivedData.size() >= 2 << 20)
// 			{
// 				LOG_E("kpm: Received data exceeds maximum size\n");
// 				break;
// 			}
// 		}
...
// 		if (valread < 0)
// 		{
// 			LOG_E("kpm: Read failed\n");
// 			exit(EXIT_FAILURE);
// 		}

// 		LOG_I("kpm: Received data: %s\n", receivedData.c_str());
// 		// Clean up and close sockets
// 		close(new_socket);
// 		close(server_fd);

// 		std::vector<uint8_t> modifiableBuffer(receivedData.begin(), receivedData.end());
// 		uint8_t *inbuf = modifiableBuffer.data();
// 		int inlen = receivedData.size();

// 		e2sim.send_sctp_data(inbuf, inlen);
// 		LOG_I("Measurement report for sequence %d has been sent\n", seqNum);

// 		// Cleanup
// 		// ASN_STRUCT_FREE(asn_DEF_E2SM_KPM_IndicationHeader, indHeader);
// 		// ASN_STRUCT_FREE(asn_DEF_E2AP_PDU, pdu);

// 		seqNum++; // Increment sequence number for the next message
// 	}
// }
...
void run_report_loop(long requestorId, long instanceId, long ranFunctionId, long actionId)
{
	...
	while (1)
	{
		...
		// E2SM_KPM_IndicationHeader_t *indHeader = 0;
		// void **indHeaderPtr = (void **)&indHeader;
		// asn_random_fill(&asn_DEF_E2SM_KPM_IndicationHeader, indHeaderPtr, 2 ^ 14);
		E2SM_KPM_IndicationHeader_t *indHeader =
		...
		kpm_report_indication_header_initialized(indHeader, plmnid_buf, sst_buf, sd_buf, fqival, qcival, nrcellid_buf, gnbid_buf, 0, cuupid_buf, duid_buf, cuupname_buf);

		if (!indHeader)
		{
			LOG_E("Failed to allocate memory for indication message or header");
			continue;
		}

		// Encode the Indication Header
		uint8_t e2smHeaderBuffer[2 << 18] = {0};
		size_t e2smHeaderBufferSize = 2 << 18;

		asn_enc_rval_t headerEncodeResult = asn_encode_to_buffer(
			NULL, ATS_ALIGNED_BASIC_PER, &asn_DEF_E2SM_KPM_IndicationHeader,
			indHeader, e2smHeaderBuffer, e2smHeaderBufferSize);

		if (headerEncodeResult.encoded == -1)
		{
			LOG_E("Failed to encode indication header");
			// ASN_STRUCT_FREE(asn_DEF_E2SM_KPM_IndicationHeader, indHeader);
			continue;
		}

		std::vector<uint8_t> modifiableBuffer(receivedData.begin(), receivedData.end());
		uint8_t *inbuf = modifiableBuffer.data(); // This gives you a modifiable uint8_t* pointer
		int inlen = receivedData.size();

		const void *constBuf = static_cast<const void *>(receivedData.data());
		const size_t constLen = receivedData.size();
		...
		// // Call aper_decode
		// asn_dec_rval_t dval = aper_decode(opt_codec_ctx, type_descriptor, struct_ptr, constBuf, constLen, 0, 0);
		// if (dval.code != RC_OK) {
		// 	LOG_E("kpm: Failed to decode message\n");
		// } else {
		// 	xer_fprint(stdout, &asn_DEF_E2SM_KPM_IndicationMessage, output);
		// }

		// Create and send the E2AP PDU
		...
		encoding::generate_e2apv1_indication_request_parameterized(
			...
			e2smHeaderBuffer, headerEncodeResult.encoded,
			inbuf, inlen);
...
void writeBytesToFile(const uint8_t *ind_message_buf, int message_length, const std::string &filename)
{
	...
	outputFile.write(reinterpret_cast<const char *>(ind_message_buf), message_length);
...
// 			writeBytesToFile(e2sm_message_buf_cucp_ue, er_message_cucp_ue.encoded, "e2sm_message_cucp_ue.bin");
...
// 			writeBytesToFile(e2sm_message_buf_style1, er_message_style1.encoded, "e2sm_message_style1.bin");
