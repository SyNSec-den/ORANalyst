...
var (
	blockCounter   uint32
	edgeCounter    uint32
	loopRunCounter uint32
)

type coverType int

const (
	block coverType = iota
	edge
	loopRun
	// for each loop, allocate a counter for whether it has been executed before, will be set to zero before each loop run
)

func instrument(pkg, fullName string, fset *token.FileSet, parsedFile *ast.File,
	info *types.Info, out io.Writer, blocks *[]CoverBlock, sonar *[]CoverBlock, blockInfo *[]BlockInfo) {
	// fmt.Printf("Instrumenting pkg=%v, fullName=%v\n", pkg, fullName)
	file := &File{
		fset:      fset,
		pkg:       pkg,
		fullName:  fullName,
	...
	if sonar == nil {
		...
		file.addImport("sync/atomic", "_atomic_", []string{"LoadUint32(&_go_fuzz_dep_.NoUse)"})
		file.addImport("go-fuzz-dep", fuzzdepPkg, []string{"CoverTab"})
		ast.Walk(file, file.astFile)
		edgeVisitor := &EdgeVisitor{
			file: file,
		}
		ast.Walk(edgeVisitor, file.astFile)
		loopVisitor := &LoopVisitor{
			file: file,
		}
		ast.Walk(loopVisitor, file.astFile)
		ast.Inspect(file.astFile, func(n ast.Node) bool {
			...
			// fmt.Println(pkg)
			// if len(nameSplit) > 2 {
			// 	mainPkg = nameSplit[len(nameSplit)-2]
			// }
			mainPkg = pkg

			callExpr := &ast.CallExpr{
				Fun: &ast.SelectorExpr{
					X:   ast.NewIdent("_go_fuzz_dep_"),
					Sel: ast.NewIdent("InstrumentMain"),
				},
				Args: []ast.Expr{},
			}

			fn.Body.List = append([]ast.Stmt{&ast.ExprStmt{X: callExpr}}, fn.Body.List...)
			return false
		})
	} else {
		// s := &Sonar{
...
func removeCover(pkg, fullName string, fset *token.FileSet, parsedFile *ast.File, info *types.Info, out io.Writer, blocks *[]CoverBlock, sonar *[]CoverBlock) {
	file := &File{
		fset:     fset,
		pkg:      pkg,
		fullName: fullName,
	...
	ast.Walk(&coverTabVisitor{}, file.astFile)
	file.print(out)
}

type coverTabVisitor struct{}

func (v *coverTabVisitor) Visit(node ast.Node) ast.Visitor {
	// Check for block statement (which contains a list of statements)
		...
		for _, stmt := range block.List {
			...
			if assignStmt, ok := stmt.(*ast.AssignStmt); ok {
				...
				for _, rh := range assignStmt.Rhs {
					// Check if right-hand side matches the pattern
					if basicLit, ok := rh.(*ast.BasicLit); ok && basicLit.Kind == token.STRING {
						value, _ := strconv.Unquote(basicLit.Value)
						if strings.HasPrefix(value, "end of CoverTab[") && strings.HasSuffix(value, "]") {
							shouldKeep = false
							break
						}
					}
...
func (f *File) Visit(node ast.Node) ast.Visitor {
	...
	case *ast.FuncLit:
		if len(n.Body.List) == 1 {
			// Inline single-statement functions are here.
		}
	case *ast.FuncDecl:
	...
	case *ast.BlockStmt:
		...
		n.List = instrumentGoRoutines(n.List)
	case *ast.IfStmt:
		if n.Init != nil {
			ast.Walk(f, n.Init)
		}
...
func instrumentGoRoutines(list []ast.Stmt) []ast.Stmt {
	for i := 0; i < len(list); i++ {
		...
		if ok {
			...
			if ok {
				...
				if ok {
					if sel.Sel.Name == "LoopPos" {
						fmt.Printf("Found LoopPos\n")
						id, ok := sel.X.(*ast.Ident)
						if ok && id.Name == "gofuzzdep" {
							fmt.Printf("Found gofuzzdep\n")
							loopFuncCall := &ast.CallExpr{
								Fun: &ast.SelectorExpr{
									X:   ast.NewIdent("_go_fuzz_dep_"),
									Sel: ast.NewIdent("LoopFunc"),
								},
							}
							list = append(list[:i], append([]ast.Stmt{&ast.ExprStmt{X: loopFuncCall}}, list[i:]...)...)
							i += 1 // Skip the newly inserted statements
							continue
						}
					}
				}
		...
		if !ok {
			// If the statement is a block statement, visit it recursively
			if blockStmt, ok := stmt.(*ast.BlockStmt); ok {
				bList := instrumentGoRoutines(blockStmt.List)
				blockStmt.List = bList
			}
			continue
		}
		...
		if ok {
			// Create a copy of the original function arguments
			var args []ast.Expr
			args = append(args, goStmt.Call.Args...)
			goStmt.Call = &ast.CallExpr{
												...
												&ast.ExprStmt{
													X: &ast.CallExpr{
														Fun: &ast.SelectorExpr{
															X: &ast.SelectorExpr{
																X:   ast.NewIdent("_go_fuzz_dep_"),
																Sel: ast.NewIdent("RoutineInfo"),
															},
															Sel: ast.NewIdent("AddTerminatedRoutineNum"),
														},
														Args: []ast.Expr{ast.NewIdent(fmt.Sprintf("_curRoutineNum%d_", routineNum))},
													},
												},
											},
										},
				...
				Lparen: goStmt.Call.Lparen, // preserve original position (optional)
				Rparen: goStmt.Call.Rparen, // preserve original position (optional)
			}
		} else {
			funLit, ok := goStmt.Call.Fun.(*ast.FuncLit)
			if ok {
				bodyList := make([]ast.Stmt, 0)
				bodyList = append(bodyList, &ast.DeferStmt{
					Call: &ast.CallExpr{
						Fun: &ast.FuncLit{
							Type: &ast.FuncType{},
							Body: &ast.BlockStmt{
				...
				bodyList = append(bodyList, funLit.Body.List...)
				funLit.Body.List = bodyList
			}
		}

		// Create new statements
		curRoutineNumStmt := &ast.AssignStmt{
			Lhs: []ast.Expr{
				ast.NewIdent(fmt.Sprintf("_curRoutineNum%d_", routineNum)),
			},
			Tok: token.DEFINE,
			Rhs: []ast.Expr{
				&ast.CallExpr{
					Fun: &ast.SelectorExpr{
						X:   ast.NewIdent("_atomic_"),
		...
		// curRoutineNumDecl := &ast.GenDecl{
		// 	Tok: token.VAR,
		// 	Specs: []ast.Spec{
		// 		&ast.ValueSpec{
		// 			Names: []*ast.Ident{{Name: "_curRoutineNum_"}},
		// 			Values: []ast.Expr{
		// 				&ast.CallExpr{
		// 					Fun: &ast.SelectorExpr{
		// 						X:   &ast.Ident{Name: "_atomic_"},
		// 						Sel: &ast.Ident{Name: "AddUint32"},
		// 					},
		// 					Args: []ast.Expr{
		// 						&ast.UnaryExpr{
		// 							Op: token.AND,
		...
		// 	},
		// }
		// curRoutineNumStmt := &ast.DeclStmt{Decl: curRoutineNumDecl}
		createdRoutinesStmt := &ast.ExprStmt{
			X: &ast.CallExpr{
				Fun: &ast.SelectorExpr{
					X: &ast.SelectorExpr{
						X:   ast.NewIdent("_go_fuzz_dep_"),
					...
					Sel: ast.NewIdent("AddCreatedRoutineNum"),
				},
				Args: []ast.Expr{ast.NewIdent(fmt.Sprintf("_curRoutineNum%d_", routineNum))},
			},
		}

		// Insert the new statements before the GoStmt
		list = append(list[:i], append([]ast.Stmt{curRoutineNumStmt, createdRoutinesStmt}, list[i:]...)...)
		i += 2 // Skip the newly inserted statements
		routineNum += 1
	}
	return list
}

func (f *File) addImport(path, name string, anyIdents []string) {
	...
	for _, anyIdent := range anyIdents {
...
func (f *File) newComment(cnt int) ast.Stmt {
	comment := &ast.AssignStmt{
		Lhs: []ast.Expr{
			ast.NewIdent("// _"),
		},
		Tok: token.ASSIGN,
		Rhs: []ast.Expr{
			&ast.BasicLit{
				Kind:  token.STRING,
				Value: strconv.Quote(fmt.Sprintf("end of CoverTab[%d]", cnt)),
			},
		},
	}

	// comment2 := &ast.IncDecStmt{
	// 	X: &ast.IndexExpr{
	// 		X: &ast.SelectorExpr{
	// 			X:   ast.NewIdent(fuzzdepPkg),
	// 			Sel: ast.NewIdent("EndCoverTab"),
	// 		},
	// 		Index: &ast.BasicLit{
	// 			Kind:  token.INT,
...
func (f *File) newEmptyStmt(cnt int) ast.Stmt {
	emptyStmt := &ast.AssignStmt{
		Lhs: []ast.Expr{
			ast.NewIdent("_"),
		},
		Tok: token.ASSIGN,
...
func appendEndComment(stmt []ast.Stmt, cmt ast.Stmt) []ast.Stmt {
	for i := len(stmt) - 1; i >= 0; i-- {
		if _, ok := stmt[i].(*ast.ReturnStmt); ok {
			stmt = append(stmt[:i], append([]ast.Stmt{cmt}, stmt[i:]...)...)
			return stmt
		}
	}
	return append(stmt, cmt)
}

func (f *File) countInstructions(id int, stmts []ast.Stmt) {
	total, ioOps, memOps, netOps := 0, 0, 0, 0
	for _, stmt := range stmts {
		ast.Inspect(stmt, func(n ast.Node) bool {
			switch x := n.(type) {
			case *ast.AssignStmt, *ast.ExprStmt, *ast.DeclStmt, *ast.GoStmt, *ast.SendStmt, *ast.IncDecStmt, *ast.BranchStmt:
				total++
			case *ast.CallExpr:
				if sel, ok := x.Fun.(*ast.SelectorExpr); ok {
					if ident, ok := sel.X.(*ast.Ident); ok {
						pkg := ident.Name
						if strings.Contains(pkg, "os") || strings.Contains(pkg, "fmt") || strings.Contains(pkg, "io") {
							ioOps++
						}
						if strings.Contains(pkg, "net") {
							netOps++
						}
					}
				}
	...
	*f.blockInfo = append(*f.blockInfo, BlockInfo{ID: id, TotalInstr: total, IOInstr: ioOps, MemInstr: memOps, NetworkInstr: netOps})
}
...
func (f *File) addCounters(pos, blockEnd token.Pos, list []ast.Stmt, extendToClosingBrace bool) []ast.Stmt {
	...
	if len(list) == 0 {
		stmt, cnt := f.newBlockCounter(pos, blockEnd, 0)
		*f.blockInfo = append(*f.blockInfo, BlockInfo{ID: cnt, TotalInstr: 0, IOInstr: 0, MemInstr: 0, NetworkInstr: 0})
		return []ast.Stmt{stmt, f.newComment(cnt)}
	}
	...
	for {
		...
		if pos != end { // Can have no source to cover if e.g. blocks abut.
			stmt, cnt = f.newBlockCounter(pos, end, last)
			newList = append(newList, stmt)
		}
		newList = append(newList, list[0:last]...)
		if cnt > -1 {
			newList = append(newList, f.newComment(cnt))
			f.countInstructions(cnt, list[:last])
			// if len(list) == 1 {
			// 	newList = append(newList[:len(newList)-1], f.newEmptyStmt(cnt), newList[len(newList)-1])
			// } else {
			// 	newList = append(newList, f.newComment(cnt))
			// }
		}
...
func genCounter(ct coverType) int {
	switch ct {
	case block:
		blockCounter++
		return int(blockCounter % BlockCoverSize)
	case edge:
		edgeCounter++
		return int(edgeCounter%EdgeCoverSize + BlockCoverSize)
	case loopRun:
		loopRunCounter++
		return int(loopRunCounter%LoopCounterSize + BlockCoverSize + EdgeCoverSize)
	default:
		panic("unsupported cover type")
	}
	// buf := []byte{byte(id), byte(id >> 8), byte(id >> 16), byte(id >> 24)}
	// hash := sha1.Sum(buf)
	// return int(uint16(hash[0]) | uint16(hash[1])<<8)
	// get the first 20 bits
	// return int(uint32(hash[0]) | uint32(hash[1])<<8 | (uint32(hash[2])&0x0F)<<16)
	// return int(id % CoverSize)
}

func (f *File) newBlockCounter(start, end token.Pos, numStmt int) (ast.Stmt, int) {
	cnt := genCounter(block)
