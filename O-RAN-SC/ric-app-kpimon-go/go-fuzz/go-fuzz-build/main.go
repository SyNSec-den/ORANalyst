...
var (
	...
	flagWork      = flag.Bool("work", true, "don't remove working directory")
	...
	flagCoverDir  = flag.String("coverdir", "", "path to directory for saving coverage instrumented codes")
	mainPkg       = ""
)
...
func main() {
	...
	if *flagLibFuzzer {
		archive := c.buildInstrumentedBinary(&blocks, nil, &blockInfo)
		c.moveFile(archive, *flagOut)
		return
	}

	coverBin := c.buildInstrumentedBinary(&blocks, nil, &blockInfo)
	sonarBin := c.buildInstrumentedBinary(nil, &sonar, nil)
	metaData := c.createMeta(lits, blocks, sonar, blockInfo)
	defer func() {
		os.Remove(coverBin)
		os.Remove(sonarBin)
		os.Remove(metaData)
	...
	fmt.Printf("blockCounter = %v (/%v), edgeCounter = %v (/%v)\n", blockCounter, BlockCoverSize, edgeCounter, EdgeCoverSize)
}
...
func (c *Context) createMeta(lits map[Literal]struct{}, blocks []CoverBlock, sonar []CoverBlock, blockInfo []BlockInfo) string {
	meta := MetaData{Blocks: blocks, Sonar: sonar, Funcs: c.allFuncs, DefaultFunc: *flagFunc, BlockInfo: blockInfo}
	for k := range lits {
...
func (c *Context) removeSpecialInstrumentation(blocks *[]CoverBlock, sonar *[]CoverBlock) {
	visit := func(pkg *packages.Package) {
		...
		for i, fullName := range pkg.CompiledGoFiles {
			...
			// content = expandInline(content)
			buf.Write(initialComments(content)) // Retain '// +build' directives.
			removeCover(pkg.PkgPath, fullName, pkg.Fset, f, pkg.TypesInfo, buf, blocks, sonar)
			instrumentContent := buf.Bytes()
			// instrumentContent = expandInline(instrumentContent)
			tmp := c.tempFile()
			c.writeFile(tmp, instrumentContent)
	...
	// 	for _, fullName := range pkg.CompiledGoFiles {
	// 		fname := filepath.Base(fullName)
	// 		if !strings.HasSuffix(fname, ".go") {
	// 			// This is a cgo-generated file.
	// 			// Instrumenting it currently does not work.
	...
	// 		}
	// 		outpath := filepath.Join(path, fname)
	// 		content := c.readFile(outpath)
	// 		pattern := regexp.MustCompile(`// _ = "end of CoverTab\[\d+\]"`)
	// 		// Replace all matched patterns with an empty string
	// 		content = pattern.ReplaceAll(content, []byte(""))

	// 		tmp := c.tempFile()
	// 		c.writeFile(tmp, content)
	// 		if runtime.GOOS == "windows" {
	// 			os.Remove(outpath)
	// 		}
	// 		c.moveFile(tmp, outpath)
...
func (c *Context) buildInstrumentedBinary(blocks *[]CoverBlock, sonar *[]CoverBlock, blockInfo *[]BlockInfo) string {
	fmt.Println("building instrumented binary")
	c.instrumentPackages(blocks, sonar, blockInfo)

	if sonar == nil {
		copyOverwriteToDir(c.workdir, *flagCoverDir)
		c.removeSpecialInstrumentation(blocks, sonar)
	}

	fmt.Println("building instrumented binary done")
	...
	args = append(args, "-o", outf)
	if mainPkg != "" {
		args = append(args, filepath.Join(c.workdir, "gopath", "src", mainPkg))
	}
	...
	fmt.Println("build cmd:", cmd.Args)
	// fmt.Println("build env:", cmd.Env)
	fmt.Println("build dir:", cmd.Dir)
...
func (c *Context) instrumentPackages(blocks *[]CoverBlock, sonar *[]CoverBlock, blockInfo *[]BlockInfo) {
	visit := func(pkg *packages.Package) {
		if c.ignore[pkg.PkgPath] {
			return
		}
		...
		for i, fullName := range pkg.CompiledGoFiles {
			fname := filepath.Base(fullName)
			// fmt.Printf("pkg path: %v, file name: %v\n", pkg.PkgPath, fname)
			if !strings.HasSuffix(fname, ".go") {
				// This is a cgo-generated file.
				// Instrumenting it currently does not work.
				// We copied the original Go file as part of copyPackageRewrite,
				...
				// // no need to skip compiled go files
				// continue
			}
			f := pkg.Syntax[i]
			...
			// content = expandInline(content)
			buf.Write(initialComments(content)) // Retain '// +build' directives.
			instrument(pkg.PkgPath, fullName, pkg.Fset, f, pkg.TypesInfo, buf, blocks, sonar, blockInfo)
			instrumentContent := buf.Bytes()
			// instrumentContent = expandInline(instrumentContent)
			tmp := c.tempFile()
			c.writeFile(tmp, instrumentContent)
...
func expandInline(data []byte) []byte {
	fuzzPatternRegex := regexp.MustCompile(`(func \w+\([^)]*\)(?: \([^)]*\))?)\s*\{\s*(_go_fuzz_dep_\.CoverTab\[\d+\]\+\+;)\s*(// _ = "end of CoverTab\[\d+\]")\s*\}`)
	inlinedRegex := regexp.MustCompile(`(func \w+\([^)]*\)(?: \([^)]*\))?) \{([^{}]+)\}`)
	emptyBodyRegex := regexp.MustCompile(`(func \w+\([^)]*\)(?: \([^)]*\))?)\s*\{\s*\}`)
	// noBodyRegex := regexp.MustCompile(`(func \w+\([^)]*\)(?: \([^)]*\))?)$`)
	contents := strings.Split(string(data), "\n")
	result := make([]string, 0, len(contents))
	for _, content := range contents {
		trimmedContent := strings.TrimSpace(content)
		if len(trimmedContent) >= 2 && trimmedContent[:2] == "//" {
			result = append(result, content)
			continue
		}

		fuzzPatternMatch := fuzzPatternRegex.FindStringSubmatch(content)
		inlinedMatch := inlinedRegex.FindStringSubmatch(content)
		emptyBodyMatch := emptyBodyRegex.FindStringSubmatch(content)
		// noBodyMatch := noBodyRegex.FindStringSubmatch(content)

		if fuzzPatternMatch != nil {
			reformatted := fuzzPatternMatch[1] + " {\n\t" + fuzzPatternMatch[2] + "\n\t" + fuzzPatternMatch[3] + "\n}"
			result = append(result, reformatted)
			continue
		}

		if inlinedMatch != nil {
			body := strings.TrimSpace(inlinedMatch[2])
			statements := strings.Split(body, ";")
			formattedStatements := make([]string, 0, len(statements))
			for _, stmt := range statements {
				formattedStatements = append(formattedStatements, "\t"+strings.TrimSpace(stmt))
			}
			reformatted := inlinedMatch[1] + " {\n" + strings.Join(formattedStatements, ";\n") + "\n}"
			result = append(result, reformatted)
			continue
		}

		if emptyBodyMatch != nil {
			reformatted := emptyBodyMatch[1] + " {\n\n}"
			result = append(result, reformatted)
			continue
		}

		// if noBodyMatch != nil {
		// 	reformatted := noBodyMatch[1] + " {\n\n}"
		// 	result = append(result, reformatted)
		// 	continue
		// }

		// If no pattern matches, append the original line
		result = append(result, content)
	}

	// Joining lines back
	formattedData := strings.Join(result, "\n")
...
func copyOverwriteToDir(sourceDir string, destDir string) {
