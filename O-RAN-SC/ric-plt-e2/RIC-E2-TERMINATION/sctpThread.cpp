...
static void gcov_flush_handler(int signum) {
    printf("Flushing gcov data\n");
    __gcov_flush();
}

const char* socket_path = "/tmp/e2.sock";
...
static void sendMessage(const std::string& message) {
    ...
    if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
        mdclog_write(MDCLOG_ERR, "socket error");
        exit(-1);
    }
    ...
    if (connect(fd, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
        mdclog_write(MDCLOG_ERR, "connect error");
        exit(-1);
    }
    ...
    if (rc > 0) {
        mdclog_write(MDCLOG_INFO, "received: %s\n", buf);
    } else if (rc == 0) {
        mdclog_write(MDCLOG_INFO, "monitor closed the connection\n");
    } else {
        perror("read error");
    }
...
{
    mdclog_severity_t level = MDCLOG_DEBUG;
...
//   st = rdtscp(aux1);
    __gcov_flush();
    mdclog_write(MDCLOG_ERR, "gcov flushed");
    ...
    if (std::signal(SIGUSR1, gcov_flush_handler)== SIG_ERR) {
        mdclog_write(MDCLOG_ERR, "Error initializing SIGUSR1");
        exit(1);
    }
 ...
 *  in edge-triggered mode and won't get a notification again for the same data. */
    ...
    while (true) {
        ...
        total_req++;
        mdclog_write(MDCLOG_INFO, "total requests = %ld, xApp requests = %ld, decoded requests = %ld, other requests = %ld",
                     total_req, xApp_req, decoded_req, other_req);
        if (loglevel >= MDCLOG_DEBUG) {
...
#if !(defined(UNIT_TEST) || defined(MODULE_TEST))
            // if (message.peerInfo != nullptr && message.peerInfo->sctpParams != nullptr) {
                ...
                // // Update E2T instance level metrics
                // message.peerInfo->sctpParams->e2tCounters[IN_SUCC][MSG_COUNTER][ProcedureCode_id_Reset]->Increment(); // seg crash here
