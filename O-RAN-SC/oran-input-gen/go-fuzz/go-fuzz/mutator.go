...
import (
	...
	"github.com/dvyukov/go-fuzz/go-fuzz/asn1_mutator"
...
func newMutator() *Mutator {
	...
	return &Mutator{r: pcg.New(), rr: rand.New(source)}
}
...
func (m *Mutator) mutate(data []byte, ro *ROData) []byte {
	r := m.r.Intn(100)
	if r < 95 {
		structRes, err := asn1_mutator.MutateStructWrapper(data)
		if err == nil {
			if r < 90 {
				log.Printf("struct mutation succeeded: %v\n", err)
				return structRes
			}
			log.Printf("going to struct byte mutation")
			data = structRes
		} else {
			log.Println(fmt.Sprintf("struct mutation failed: %v, falling back to byte mutation\n", err))
		}
	} else {
		log.Printf("going to direct byte mutation\n")
	}
	corpus := ro.corpus
