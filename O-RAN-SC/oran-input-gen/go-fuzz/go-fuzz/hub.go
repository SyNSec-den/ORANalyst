...
func newHub(metadata MetaData) *Hub {
	...
	hub.maxCover.Store(cover)

	coverHistory := make([][]byte, CoverSize)
	for i := range cover {
		coverHistory[i] = make([]byte, 2)
	}

	ro := &ROData{
		corpusCover:  coverHistory,
	...
	for {
		...
		case input := <-hub.newInputC:
			...
			*ro1 = *ro
			...
			ro1.corpusCover = makeHistCopy(ro.corpusCover)
			hub.corpusCoverSize = updateMaxCover(ro1.corpusCover, input.cover)
			log.Printf("hub corpusCoverSize=%v", hub.corpusCoverSize)
...
func (hub *Hub) updateMaxCover(cover []byte) bool {
	...
	maxCover := makeHistCopy(oldMaxCover)
