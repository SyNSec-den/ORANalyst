...
func workerMain() {
	...
	// }
	if sonarBin == "" || len(metadata.Blocks) == 0 {
		log.Fatalf("bad input archive: missing file")
	}
...
func (w *Worker) loop() {
	...
	for atomic.LoadUint32(&shutdown) == 0 {
		...
		if iter%10 != 0 || ro.verse == nil {
			log.Printf("worker %v generates", w.id)
...
func (w *Worker) triageInput(input CoordinatorInput) {
	...
	for i := 0; i < 3; i++ {
		...
		res, ns, cover, _, output, crashed, hanged := w.sonarBin.test(inp.data)
		if crashed {
	...
	w.hub.newInputC <- inp
	log.Printf("triaged input complete")
}
...
func (w *Worker) minimizeInput(data []byte, canonicalize bool, pred func(candidate, cover, output []byte, result int, crashed, hanged bool) bool) []byte {
	...
	for n := 1024; n != 0; n /= 2 {
		for len(res) > n {
			...
			*stat++
			result, _, cover, _, output, crashed, hanged := w.sonarBin.test(candidate)
...
func (w *Worker) testInput(data []byte, depth int, typ execType) {
	w.testInputImpl(w.sonarBin, data, depth, typ)
}
