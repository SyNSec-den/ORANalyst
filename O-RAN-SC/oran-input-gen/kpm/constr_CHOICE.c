...
asn_mutate_result_t
CHOICE_mutate(const asn_TYPE_descriptor_t *td, void **sptr,
                   const asn_encoding_constraints_t *constr) {
	printf("CHOICE_mutate\n");
    const asn_CHOICE_specifics_t *specs =
        (const asn_CHOICE_specifics_t *)td->specifics;
    ...
    if(st == NULL) {
        asn_random_fill_result_t gen_res = CHOICE_random_fill(td, sptr, constr, 1048576);
		if(gen_res.code != ARFILL_OK) {
