...
import (
	...
	e2sm_rcpre "github.com/onosproject/onos-e2-sm/servicemodels/e2sm_rc_pre_go/v2/e2sm-rc-pre-v2-go"
...
func rc_pre() {
	...
	for {
		...
		indMsg := &e2sm_rcpre.E2SmRcPreIndicationMessage{}
		...
		asn1Ind, err := rcPreMessageToAsn1Bytes(indMsg)
		...
		_, err = rcPreMessageToProtoBytes(asn1Ind)
...
func rcPreHeaderToAsn1Bytes(header *e2sm_rcpre.E2SmRcPreIndicationHeader) ([]byte, error) {
	indicationHeaderProtoBytes, err := proto.Marshal(header)
	if err != nil {
		return nil, err
	}

	var rcServiceModel e2smrcpresm.RcPreServiceModel
...
func rcPreMessageToAsn1Bytes(msg *e2sm_rcpre.E2SmRcPreIndicationMessage) ([]byte, error) {
