...
var CumulativeCover []byte = make([]byte, CoverSize)
...
func main() {
	...
	t := startTestee(sonarBin, mappingFile, mem)
	t.watchResult(mem)
}

func setupCoverServer() {
	coverServer, err := net.Listen("tcp", ":19999")
	if err != nil {
		panic(err)
	}
	fmt.Println("start cover server")
	coverConn, err = coverServer.Accept()
	if err != nil {
		panic(err)
	}
...
func startTestee(sonarBin string, mappingFile *Mapping, mem []byte) *testee {
retry:
	rIn, wIn, err := os.Pipe()
	if err != nil {
...
func (t *testee) watchResult(mem []byte) {
	defer t.shutdown()
	for {
		...
		for i, v := range t.coverRegion {
			if v > CumulativeCover[i] {
				CumulativeCover[i] = v
			}
		}
		...
		if err != nil {
			log.Fatalf("failed to write to cover server: %v", err)
		}
