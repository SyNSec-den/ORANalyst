...
func OnosE2tIndication() {
	...
	for i := 0; i < flag.NArg(); i++ {
		...
		for name, pkg := range progInfoGraph.ContainsPkg {
			if inputType == nil {
				// inputType, _ = path_collection.LookUpTypeFromPackage(pkg.PackagePtr, "RanE2tMap")
				inputType, _ = condition_collection.LookUpTypeFromPackage(pkg.PackagePtr, "Ricindication")
		...
		cc := condition_collection.NewConditionCollector("onos-e2t", []types.Type{inputType}, nil, inputPackage)
...
func OnosE2tE2Setup() {
	...
	for i := 0; i < flag.NArg(); i++ {
		...
		var inputType types.Type
		var inputPackage packages.Package
		processFuncMap := make(map[*PdgGraph.Function]bool)
		for name, pkg := range progInfoGraph.ContainsPkg {
			if inputType == nil {
				inputType, _ = condition_collection.LookUpTypeFromPackage(pkg.PackagePtr, "E2SetupRequest")
				inputPackage = *pkg.PackagePtr
			}
			if name == "github.com/onosproject/onos-e2t/pkg/southbound/e2ap/server" {
				for funcName, funcInfo := range pkg.ContainsFunction {
			...
			if name == "github.com/onosproject/onos-e2t/pkg/southbound/xnap/xnutils" {
				for funcName, funcInfo := range pkg.ContainsFunction {
					if funcName == "GetTopoNeighborInformationNR" {
						processFuncMap[funcInfo] = false
					}
				}
			}
		}
